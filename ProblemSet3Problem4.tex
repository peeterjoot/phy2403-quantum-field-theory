...
\end{aligned}
\end{dmath}
These zeros are easily computed by commutation, but also by the Wick's corollary mentioned in class (expectations of odd numbers of creation or anhillation operators are zero).
With those same sign \((p,q)\) expoential terms eliminated and a \( p, q \) swap in the \( a_\Bp a_\Bq^\dagger \) term, we are left with
