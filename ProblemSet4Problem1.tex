...
Considering the \( n = 2 \) case again, with \( A_i = \phi(x), A_j = \phi(y) \) defined by
\cref{eqn:ProblemSet4Problem1:360}, we clearly have
...
A B,
\end{dmath}
where the contraction is now defined by \cref{eqn:ProblemSet4Problem1:440}.  We now have to ask what the meaning of a normal ordered time ordered product is.  The \( n = 2 \) case illustrates this nicely.  If \( y^0 > x^0 \), then
\begin{dmath}\label{eqn:ProblemSet4Problem1:480}
\normalorder{ T\lr{ \phi(x) \phi(y)} }
=
\normalorder{ \phi(y) \phi(x) }
=
\bra{0}
\int \frac{d^3 p d^3 q}{(2 \pi)^6 \sqrt{ 2 \omega_\Bp 2 \omega_\Bq}}
\normalorder{
...
\normalorder{ T( \phi(x) \phi(y) }
=
\normalorder{ \phi(x) \phi(y) }
=
\bra{0}
\int \frac{d^3 p d^3 q}{(2 \pi)^6 \sqrt{ 2 \omega_\Bp 2 \omega_\Bq}}
\normalorder{
...
\ket{0},
\end{dmath}
which equals \cref{eqn:ProblemSet4Problem1:480}, that is
\begin{dmath}\label{eqn:ProblemSet4Problem1:n}
\normalorder{ T\lr{ \phi(x) \phi(y)} }
=
\normalorder{ T\lr{ \phi(x) \phi(y)} }
\end{dmath}
so we see that the time ordering of a normal ordered product, such as \( \normalorder{ T(A B) } = \normalorder{ T(B A) } \), is redundant.  Such an argument clearly extends to multiple products.  We also need this argument to extend to contractions against the fields in a normal ordering product.  In particular, for three fields \( A, B, C \)
...
=
T( \lr{ \normalorder{A B} } C ) + (\contraction{}{A}{}{B} A B) C,
\end{dmath}
so we see that proving the time ordered Wick's theorem requires that we prove a variation of our lemma \cref{lemma}, which for the \( n = 2 \) case, is
