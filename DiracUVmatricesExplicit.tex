...
It happens that \( 2 \times 2 \) matrices can be square-rooted symbolically \cref{path:phy2403-quantum-field-theory:squarerootOfFourSigmaDotP.nb}.  In particular, the matrices \( p \cdot \sigma, p \cdot \osigma \)  have nice simple eigenvalues \( \pm \Norm{\Bp} + \omega_\Bp \).  The corresponding unnormalized eigenvectors for \( p \cdot \sigma \) are
...
\Cref{eqn:squarerootpsigma:220} is probably a much nicer starting point for evaluating the various \( u, v, \ubar, \vbar \) product relationships.  In particular, again using Mathematica \cref{path:phy2403-quantum-field-theory:uvspinors.nb}, this is a nice representation for showing that
