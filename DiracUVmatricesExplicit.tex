...
It happens that \( 2 \times 2 \) matrices can be square-rooted symbolically \nbref{squarerootOfFourSigmaDotP.nb}.  In particular, the matrices \( p \cdot \sigma, p \cdot \osigma \)  have nice simple eigenvalues \( \pm \Norm{\Bp} + \omega_\Bp \).  The corresponding unnormalized eigenvectors for \( p \cdot \sigma \) are
...
\section{Compact representation of sigma roots.}
...
\begin{equation}\label{eqn:DiracUVmatricesExplicit:280}
\begin{aligned}
...
\begin{equation}\label{eqn:DiracUVmatricesExplicit:300}
\begin{aligned}
\sqrt{ p \cdot \sigma }
&=
...
\begin{equation}\label{eqn:DiracUVmatricesExplicit:340}
\begin{aligned}
...
\Cref{eqn:DiracUVmatricesExplicit:360} is probably a much nicer starting point for evaluating the various \( u, v, \ubar, \vbar \) product relationships.  In particular, again using Mathematica \nbref{uvspinors.nb}, this is a nice representation for showing that
\begin{equation}\label{eqn:DiracUVmatricesExplicit:260}
\begin{aligned}
...
\begin{equation}\label{eqn:DiracUVmatricesExplicit:380}
\begin{aligned}
u^1(p) &=
\inv{\sqrt{ 2 m + 2 \omega_\Bp }}
\begin{bmatrix}
