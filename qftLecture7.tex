...
\beginArtNoToc
\generatetitle{PHY2403H Quantum Field Theory.  Lecture 7: Symmetries.  Taught by Prof. Erich Poppitz}
%\chapter{Symmetries}
...
\end{dmath}
where \( \delta \phi^i \) is considered to be an infimesimal transformation.
\end{itemize}
...
\end{equation*}
where
\( \delta \phi^i \) is an infimesimal transformation, and the equations of motion are invariant under this transformation.
} % definition
...
FIXME: missed some stuff related to Q here.
%% missed some stuff here.
%0 = \int d^3 \Bx j^0(\Bx, t)
%% defined Q.

\section{Spacetime translation.}

A spacetime translation has the form
\begin{dmath}\label{eqn:qftLecture7:320}
x^\mu \rightarrow {x'}^\mu = x^\mu + a^\mu,
\end{dmath}
\begin{dmath}\label{eqn:qftLecture7:340}
...
Change of the Lagrangian in the \( \nu \) direction
\begin{dmath}\label{eqn:qftLecture7:460}
\delta_\nu \LL = -\partial_\nu \LL
= - \partial_\mu \lr{ \delta^\mu_\nu \LL )
= \partial_\mu {J^\mu}_\nu
\end{dmath}
we write
...
\end{dmath}
where \( \nu \) are lables which coordinates are translated:
