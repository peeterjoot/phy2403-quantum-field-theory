...
%
\makeoproblem{Playing with the non-relativistic limit.}{qft:problemSet2:4}{2018 Hw2.IV}{
...
\index{non-relativisitic limit}

{\small {\tt Hint}: For
\cref{qft:problemSet2:4d},
% part 4,
consider $N=2$ first. Start with a two particle nonrelativistic quantum mechanics with Hamiltonian:
\begin{equation}\label{eqn:ProblemSet2Problem4:220}
H = {p_1^2\over 2m} + {p_2^2\over 2m} + V(x_1 - x_2)~,
\end{equation}
where $p_i, x_i$ are the operators of momentum and position of the $i-th$ particle (three vectors, arrows omitted for brevity).
Use as a basis the eigenstates of the free Hamiltonian, i.e. plane waves, $| \vec{p}_1, \vec{p}_2 \rangle$, symmetric with respect to interchange of the momenta (even better, use the corresponding wavefunctions $\psi_{p_1,p_2}(x_1, x_2) = \langle x_1 , x_2 | p_1, p_2\rangle$). Compute the matrix elements
\begin{equation}\label{eqn:ProblemSet2Problem4:240}
\langle q_1, q_2 | H | p_1, p_2 \rangle
\end{equation}
in this basis. To compare to the nonrelativistic limit of the scalar field theory, compute the same matrix elements of the Hamiltonian you found in (1.) above, in the basis of states of the restricted  ($N=2$) Hilbert space $|p_1, p_2\rangle$. Are they similar to the matrix elements you found in the quantum mechanics problem for some choice of $V(x_1 - x_2)$? Explain the difference (if any). Then go on to answer (4.) for any $N$.
}
} % makeproblem
...
\begin{equation}\label{eqn:ProblemSet2Problem4:40}
...
\lr{ a_\Bp e^{-i p \cdot x} + a_\Bp e^{i p \cdot x} } \,\times \\
&\qquad
...
\\
&\qquad
   a_\Bp a_\Bk a_\Bq a_\Bs e^{-i (\omega_\Bp + \omega_\Bk + \omega_\Bq + \omega_\Bs)t} \deltathree( \Bp + \Bk + \Bq + \Bs )
\\
&\qquad
   +
   a_\Bp a_\Bk a_\Bq a_\Bs^\dagger e^{-i (\omega_\Bp + \omega_\Bk + \omega_\Bq - \omega_\Bs)t} \deltathree( \Bp + \Bk + \Bq - \Bs )
   +
   \cdots
\\
&\qquad
   +
   a_\Bp^\dagger a_\Bk^\dagger a_\Bq^\dagger a_\Bs^\dagger e^{-i (-\omega_\Bp - \omega_\Bk - \omega_\Bq - \omega_\Bs)t} \deltathree( -\Bp - \Bk - \Bq - \Bs )
} \\
&=
...
\begin{equation}\label{eqn:ProblemSet2Problem4:100}
\begin{aligned}
H_{\text{int}}
&=
\lambda \int \frac{ d^3 p d^3 k d^3 q }{4 (2 \pi)^{9} }
...
takes the form
\begin{equation}\label{eqn:ProblemSet2Problem4:120}
\begin{aligned}
\text{\(:H:\)} &=
...
However, we haven't used the non-relativistic (low energy) constraint.  That constraint can be expressed as \( \Bp^2 \ll m^2 \), in which case
\begin{equation}\label{eqn:ProblemSet2Problem4:260}
\omega_\Bp = \sqrt{ \Bp^2 + m^2 } \sim m,
\end{equation}
the mass of each of the particles.  Incorporating that into our N-particle Hamiltonian, we have
\begin{equation}\label{eqn:ProblemSet2Problem4:140}
\text{\(:H:\)} =
\sum_{i = 1}^N \omega_{\Bp_i} a_{\Bp_i}^\dagger a_{\Bp_i}
+
\frac{
...
\begin{equation}\label{eqn:ProblemSet2Problem4:160}
\text{\(:H:\)} =
\sum_{i = 1}^N \omega_{\Bp_i} a_{\Bp_i}^\dagger a_{\Bp_i}
+
\frac{
...
Yes, with the number of creation and annihilation operators matched, this Hamiltonian preserves particle number (one particle is created for each particle destroyed).
...
%\begin{equation}\label{eqn:ProblemSet2Problem4:180}
...
%\begin{equation}\label{eqn:ProblemSet2Problem4:200}
%      a_{\Bp_j}^\dagger a_{\Bp_m}^\dagger a_{\Bp_n} a_{{\Bp_j} + {\Bp_m} - {\Bp_n}}
%\ket{\Bp_1, \Bp_2, \cdots \Bp_N}.
%\end{equation}
