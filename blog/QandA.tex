...
\end{equation*}
so
\begin{equation}\label{eqn:facebook:20}
\begin{aligned}
j^{\mu, a}_L
&=
\frac{i}{2}
...
\section{blah}
\begin{equation}\label{eqn:facebook:540}
%\delta^\brn{3}(x - y)
\deltathree(x - y)
\end{equation}
\section{GA q.}
\begin{equation}\label{eqn:QandA:560}
\begin{aligned}
...
\end{aligned}
\end{equation}
\begin{equation}\label{eqn:QandA:580}
\begin{aligned}
...
I ask because I got confused looking at Peskin and Schroeder, where they seemingly use an opposite sign convention where \( \sigma \cdot x \) was first defined, namely eq. 3.41 and its neighbors where they write
\begin{equation*}
...
I'm sure that we agree that \( \int_{-\infty}^\infty f(x) \delta(x - a) = f(a) \).  Correct?
But we also have \( \int_{-\infty}^\infty f(x) (\Theta(x - a + \epsilon) - \Theta( x - a - \epsilon)) \delta(x - a) = f(a) \) -- i.e. we don't change the result we multiply the function by rectangular box of unit height of width \( 2 \epsilon > 0 \) centered on \( a \).

That second integral is just \( \int_{a - \epsilon}^{a + \epsilon} f(x) \delta( x - a ) \).
