...
\maketheorem{Creation and anhillation form of the momentum operator.}{thm:qftLecture22:560}{
...
To show this, we can plug in the field representations (\citep{peskin1995introduction} eq. 3.99, 3.100)
...
\lr{ b_\Bq^r \vbar^r(q) e^{-i q \cdot x} + a_\Bq^{s \dagger} \ubar^r(q) e^{i q \cdot x} },
\end{aligned}
\end{dmath}
to find
...
=
\int \frac{d^3 p}{ (2 \pi)^3 2 \omega_\Bp } \sum_{r,s = 1}^2
\lr{
-  b_{-\Bq}^r
   a_\Bp^s
   \vbar^r(-\Bp)
...
\( v^{r \dagger}(-\Bp) u^s(\Bp) = u^{r\dagger}(\Bp) v^s(-\Bp) = 0\)\footnote{For the KG field we had to work much harder to argue those cross terms away in the momentum operator.}, and the rest simplify using
...
P^k
=
\int \frac{d^3 p}{ (2 \pi)^3 } \sum_{r = 1}^2
\lr{
\lr{
   b_\Bp^{r \dagger}
   b_\Bp^r
...
\end{dmath}
Discarding the vacuum energy term completes the proof.

The momentum operator is also an energy eigenoperator for a single particle momentum state
