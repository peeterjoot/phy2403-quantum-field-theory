...
\end{aligned}
\end{dmath}
the frequency dependent cross terms are killed\footnote{For the KG field we had to work much harder to argue those cross terms away in the momentum operator.}, and the rest simplify to give
...
\end{aligned}
\end{dmath}
For rotation around \( \zcap \) only \( \omega_{12} \) is non-zero.  We also have \( S^{12} = S^{21} \) and
...
\end{dmath}
so
\begin{dmath}\label{eqn:qftLecture22:820}
-\frac{i}{2} \omega_{\mu\nu} S^{\mu\nu}
=
-i \omega_{12} S^{12}
=
