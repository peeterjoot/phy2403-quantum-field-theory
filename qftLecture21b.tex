...
\Cref{thm:qftLecture21b:2}, which shows that \( \Psi_u, \Psi_v \) are eigenvectors of the operator
\( \gamma^0 \lr{ -i \gamma^j \partial_j + m }\) with eigenvalues \( \pm \omega_\Bp \).
These eigenvalue equations follow from the Dirac equation for \( \Psi_u, \Psi_v \).  These are
\begin{dmath}\label{eqn:qftLecture21b:1000}
...
\end{dmath}
and
\begin{dmath}\label{eqn:qftLecture21b:1020}
...
\end{dmath}
Rearranging gives
\begin{dmath}\label{eqn:qftLecture21b:1040}
\begin{aligned}
...
As with the KG equation, let's introduce a generic solution formed from linear combinations of our specific \( u^s(p) = u^s_\Bp, v^s(p) = v^s_\Bp \) solutions
...
\end{dmath}
\maketheorem{Dirac Hamiltonian in terms of creation and anhillation operators.}{thm:qftLecture21b:7}{
Substution of the superposition \cref{eqn:qftLecture21b:800} into
the Dirac Hamiltonian of
...
\begin{dmath}\label{eqn:qftLecture21b:1080}
H_{\text{Dirac}}
=
\sum_{r,s = 1}^2
...
\begin{dmath}\label{eqn:qftLecture21b:1120}
H_{\text{Dirac}}
=
\inv{2} \sum_{r,s = 1}^2
\int \frac{d^3 p }{(2\pi)^3 }
...
\imageFigure{../figures/phy2403-quantum-field-theory/diracSeaFig1}{Dirac Sea.}{fig:diracSea:diracSeaFig1}{0.4}
Such an argument does not work for Boson's (photon's, ...) since an arbitrary number of such particles can be stuffed into any given state.  It will turn out that our operators are Fermions, which gets us out of this trouble.
...
\end{aligned}
\end{dmath}
It turns out that some properties of our creation and anhillation operators are
...
\makeproblem{Derive the Dirac Hamiltonian without using the zero-time field substitution.}{problem:qftLecture21b:1}{
} % problem

\makeanswer{problem:qftLecture21b:1}{
With time left in the mix the fields are
\begin{dmath}\label{eqn:qftLecture21b:1180}
\begin{aligned}
\Psi(x)
&=
\sum_{s = 1}^2
...
\begin{dmath}\label{eqn:qftLecture21b:1200}
H_{\text{Dirac}}
=
\sum_{r,s = 1}^2
\int \frac{d^3 x d^3 p d^3 q}{(2 \pi)^6 2 \sqrt{ \omega_\Bp \omega_\Bq } }
