...
\end{aligned}
\end{equation}
The \( \sin \) term can be interpreted as a \( \sinc \) like function of \( \omega_\Bp \) which vanishes for large \( \Bp \).  It's not entirely \( \sinc \) like for a massive field as \( \omega_\Bp = \sqrt{ \Bp^2 + m^2 } \), which never hits zero, as shown in \cref{fig:sortOfSincFunction:sortOfSincFunctionFig1}.
...
\mathImageFigure{../figures/phy2403-quantum-field-theory/sortOfSincFunctionFig1}{Angular frequency dependent sinc.}{fig:sortOfSincFunction:sortOfSincFunctionFig1}{0.2}{sincVsSineLargeT.nb}
Vanishing for large \( \Bp \) doesn't help the whole integral vanish, but we can resort to the Riemann-Lebesque lemma \citep{wiki:RiemannLebesgue} instead and interpret this integral as one with a plain old high frequency oscillation that is presumed to vanish (i.e. the rest is well behaved enough that it can be labeled as \( L_1 \) integrable.)
