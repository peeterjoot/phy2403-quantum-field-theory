...
\beginArtNoToc
\generatetitle{PHY2403H Quantum Field Theory.  Lecture 11: XXX.  Taught by Prof.\ Erich Poppitz}
%\chapter{XXX}
...
Start with \( t > 0 \), so that \( e^{-i p t} = e^{-i p \Abs{t}} \) which means that we have to integrate over the contour
F8a
because the imaginary part of \( p \) is negative, but for \( t < 0 \)
F8b
...
The question of contours can now be justified by the result, since this satisfies \cref{eqn:qftLecture11:440}.

Fixme: Exersize: CHECK.
