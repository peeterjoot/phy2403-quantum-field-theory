...
\partial_\mu H^\dagger \partial^\mu H,
\end{dmath}
since \( \partial_\mu H^\dagger \partial^\mu H \) is a scalar.  Similarily
...
\end{dmath}
where \( \trace{(ABC)} = \trace{(BCA)} = \trace{(CAB)} \) was used coerse this result into the desired form.  An assignment \( j^\mu_R = \omega_a j^{\mu,a}_R \) completes the proof.
