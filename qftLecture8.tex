...
%%%\end{dmath}
To find Noether currrent, we need to vary the field and it's derivatives
...
\end{dmath}
where the last step assumes that \( x' \rightarrow x, \phi' \rightarrow \phi \), effectively weeding out any terms that are quadradic or higher in \( \lambda \).
...
\end{dmath}
so
\begin{dmath}\label{eqn:qftLecture8:n}
...
\delta \LL 
=
(\partial_\mu \phi) \delta (\partial^\mu \phi)
= - \lambda \lr{ 2 + x^\alpha \partial_\alpha } \phi \lr{
\partial^\mu \phi 
}
=
