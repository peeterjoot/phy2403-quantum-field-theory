...
=
\int d^3 p
e^{i \Ba \cdot \hat{\Bp}} \braket{\Bx}{\Bp} \psi(\Bp)
=
\int \frac{d^3 p}{(2 \pi)^3}
e^{i \Ba \cdot \hat{\Bp}} e^{i \Bx \cdot \Bp} \psi(\Bp)
= \psi(\Bx + \Ba).
%e^{i \Ba \cdot \hat{\BP} } \psi(\Bx) = \psi(\Bx + \Ba),
\end{dmath}
Implicitly, this shows that the action of the translation operator on just a bra is
\begin{dmath}\label{eqn:qftLecture8:1560}
\bra{\Bx} \hatU(\Ba)
=
\bra{\Bx + \Ba},
\end{dmath}
or
\begin{dmath}\label{eqn:qftLecture8:1580}
\hatU(-\Ba) \ket{\Bx}
=
