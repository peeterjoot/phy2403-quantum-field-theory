...
We will only show one direction, but you can go the other too.
...
e^{i \Bp \cdot \Bx + i \Bq \cdot \Bx}
\lr{
-i \omega_\Bq (2 \pi)^3 \delta^3(\Bp + \Bq)
+ ...
}
=
-i \int \frac{d^3 p}{(2\pi)^3
...
-i \delta^3(\By - \Bx),
\end{dmath}
which is what we wanted to prove.  (FIXME: Fill in the details).

\section{Free Hamiltonian}
We call the \( \lambda = 0 \) case the ``free'' Hamiltonian.

\begin{dmath}\label{eqn:qftLecture6:300}
H = \int d^3 x \lr{ \inv{2} \pi^2 + \inv{2} (\spacegrad \phi)^2 + \frac{m^2}{2} \phi^2 }
=
\inv{2} \int d^3 x
\frac{d^3 p}{(2\pi)^3 \sqrt{2 \omega_\Bp}} e^{i \Bp \cdot \Bx}
\frac{d^3 p}{(2\pi)^3 \sqrt{2 \omega_\Bq}} e^{i \Bq \cdot \Bx}
\lr{
-(\omega_\Bq)^2
   -e^{-i \omega_\Bq t} a_\Bq
   +
...
\lr{
   \lr{
   -i \omega_\Bp e^{-i\omega_\Bp t} a_{\Bp} + i \omega_\Bp e^{i \omega_\Bp t} a^\dagger_{-\Bp}
   }
   \lr{
   -i \omega_\Bp e^{-i\omega_\Bp t} a_{-\Bp} + i \omega_\Bp e^{i \omega_\Bp t} a^\dagger_{\Bp}
   }
   ...
   \lr{ \Bp^2 + m^2 }
   \lr{
      e^{-i\omega_\Bp t} a_{\Bp} + e^{i \omega_\Bp t} a^\dagger_{-\Bp}
   }
   \lr{
      e^{-i\omega_\Bp t} a_{-\Bp} + e^{i \omega_\Bp t} a^\dagger_{\Bp}
   }
...
\lr{
   a_\Bp a_{-\Bp}
   \lr{
      \cancel{\omega_p^2 e^{-2 i \omega_\Bp t}}
      +
      \cancel{\omega_p^2 e^{-2 i \omega_\Bp t}}
   }
+
...
+
   a_\Bp a^\dagger{\Bp} \omega^2_\Bp(1 + 1)
+
...
\end{dmath}
and finally with a \( \Bp \rightarrow -\Bp \) transformation in the first integral our free Hamilitonian \( \lambda = 0 \)
...
\end{dmath}
so when \( \Bq = 0 \)
\begin{dmath}\label{eqn:qftLecture6:420}
...
\begin{dmath}\label{eqn:qftLecture6:640}
...
\end{dmath}
We say that \( a^\dagger_\Bp a^\dagger_\Bq \ket{0} \) a two particle space with energy \( \omega_p + \omega_q\), and \( (a^\dagger_\Bp)^m (a^\dagger_\Bq)^n \ket{0} \) a \( m + n \) particle space.

There is a tie to stat mech of interest
