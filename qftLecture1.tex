...
\generatetitle{UofT QFT Fall 2018 Lecture 1, What is a field?.  Taught by Prof. Erich Poppitz}
...
Examples of fields:
\begin{enumerate}
\item \( 0 + 1 \) dimensional ``QFT'', where the spatial dimension is zero dimensinal and we have one time dimension.  Fields in this case are just functions of time \( x(t) \).  That is, particle mechanics is a 0 + 1 dimensional classical field theory.  We know that classical mechanics is described by the action
...
\end{dmath}
This will be particularily useful for QFT, despite the fact that such a sum is really hard to evaluate (try it for the Hyrdogen atom for example).
...
In particular, self-accelerating solutions appear at these frequencies in classical EM.  This is appriximately \( \omega_\conj \sim 10^{23} Hz \), or
...
\end{aligned}
\end{dmath}
The Coloumb force (in cgs units) on the electron is
...
Similarily, when the length scales are reduced to the Compton wavelength, the momentum increases to relativistic levels.
Because of the relativistic velocities at the Compton wavelength, particle creation and anhination occurs and any theory has to account for multiple particle states.
...
\begin{dmath}\label{eqn:qftLecture1:n}
\lambda_\txtc \alpha
=
\frac{e^2}{4 \pi m c^2}
\sim
