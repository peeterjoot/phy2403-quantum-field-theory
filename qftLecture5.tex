...
\end{dmath}
For our Klien-Gordon system, this is
...
\end{dmath}
and that
HP time evoluation operators \( O \) satisfied
...
\section{Fourier representation of the momentum .}
...
\end{aligned}
\end{dmath}
Observe that \( \phi^\conj(\Bx, t) = \phi(\Bx, t) \) iff \( \tilde{\phi}(\Bp, t) = \tilde{\phi}^\conj(-\Bp, t) \).
...
Want the EOM for \( \tilde{\phi}(\Bp, t) \) where the operator obeys the KG equation
...
\ddot{\tilde{\phi}}(\Bq, t) = - \omega_\Bq^2 \tilde{\phi}(\Bq, t).
}
...
\tilde{\phi}(\Bq, t) = \inv{\sqrt{2 \omega_q}} \lr{ e^{-i \omega_\Bq t } a_\Bq + e^{i \omega_\Bq t} a^\dagger{-\Bq}
}
\end{dmath}

CHECK!! :
\begin{dmath}\label{eqn:qftLecture5:620}
\tilde{\phi}^\conj(\Bq, t) = \tilde{\phi}(-\Bq, t).
\end{dmath}
...
\antisymmetric{\Pihat(\By, t)}{\tilde{\phi}(\Bx, t)} = -i \delta^3(\Bx - \By)
\end{dmath}
