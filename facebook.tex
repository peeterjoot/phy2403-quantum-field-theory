...
Further confounding me was the fact that my old QM book used a different sign convention for the translation operator (\citep{desai2009quantum} uses \( D(\Ba) = e^{-i \Ba \cdot \hat{\BP}/\Hbar} \) defined by the property \( D(\Ba) \ket{\Bx} = \ket{\Bx + \Ba} \), instead of \( \bra{\Bx + \Ba} = \bra{\Bx} U(\Ba) \) as we found.)
